...
static int client_append_oro(sd_dhcp6_client *client, uint8_t **buf, size_t *offset) {
        ...
        /* Room for the requested options, the at most two options appended automatically below, and the
        ...
        if (client->address_registration.enabled)
                p[n++] = htobe16(SD_DHCP6_OPTION_ADDR_REG_ENABLE); /* RFC 9686 section 4.4 */
