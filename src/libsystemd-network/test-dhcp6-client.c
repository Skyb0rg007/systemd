...
static void test_client_verify_oro(
                const uint8_t *packet, size_t len, unsigned expect_n_address_registration, size_t n_automatic_options) {
        ...
        while (offset < len) {
                ...
                n_oro++;
                ASSERT_EQ(optlen, (1 + n_automatic_options + expect_n_address_registration) * sizeof(be16_t));
...
TEST(client_oro_address_registration) {
        ...
        /* Covers all four combinations of RegisterAddresses= and an explicit RequestOptions=148: the
         * option must appear whenever either one asks for it. When both do, it appears twice -- listing an
         * option in the ORO more than once is harmless, so nothing filters the redundant entry out. */
...
TEST(address_registration_capability) {
        ...
        /* RFC 9686 section 4.4: once registration has started the client must not stop "until it
         * disconnects from the link". Restarting DHCPv6 on the same attachment must not discard
         * discovered support. */
...
TEST(address_registration_survives_same_link_restart) {
        ...
        /* RFC 9686 section 4.4 forbids stopping registration short of a link disconnect, and stopping the
         * DHCPv6 client is not one: networkd restarts it on the same link when an RA switches it to
         * managed mode, when the MAC changes, and when the IPv6LL source address changes. Everything --
         * discovered support, the socket, the in-flight transaction and the refresh -- must survive. */
        ASSERT_OK(sd_dhcp6_client_stop(client));
        ASSERT_TRUE(client->address_registration.supported);
        ASSERT_GE(client->address_registration.fd, 0);
        ASSERT_TRUE(registration->transaction_active);
        ...
        /* A later Reply without the option must not restart it either, and must not re-register. */
        ASSERT_EQ(dhcp6_client_address_registration_discover_at(
                          client, DHCP6_MESSAGE_REPLY, /* advertised= */ false, now_usec), 0);
        ASSERT_TRUE(client->address_registration.supported);
        ASSERT_EQ(test.n_sent, 1U);

        /* Only an explicit reset -- what networkd does on carrier loss -- tears it down. */
        dhcp6_client_address_registration_reset(client);
        ASSERT_FALSE(client->address_registration.supported);
        ASSERT_EQ(client->address_registration.fd, -EBADF);
        ASSERT_FALSE(registration->transaction_active);
...
TEST(address_registration_unupdated_finite_lifetime) {
        ...
        /* RFC 9686 section 4.6.1 computes NextAddrRegRefreshTime on registration "but does not schedule
         * any refreshes". Nothing ever updates this address's lifetime, so it expires exactly when the
         * server expects it to and no refresh is due -- arming one here would only add multicast traffic
         * the specification explicitly rules out. */
        ASSERT_EQ(registration->next_refresh_usec, start_usec + 72 * USEC_PER_SEC);
        ASSERT_EQ(registration->refresh_deadline_usec, USEC_INFINITY);
        ASSERT_EQ(dhcp6_client_address_registration_refresh_at(
