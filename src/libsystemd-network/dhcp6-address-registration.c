...
/* The key lives inside the value, so the hashmap must not outlive the registrations it points at. Let it
 * own them instead of freeing them out from under it. */
...
void dhcp6_client_set_address_registration_parameters(
         ...
         * infinity is a meaningful interval, so callers reject those before they get here. */
...
int dhcp6_client_process_address_registration_reply_at(
        ...
        /* RFC 9686 section 4.3 discards the reply unless the destination address, the interface it
         * arrived on, the transaction ID and the IA Address option all match the address being
         * registered. The lookup below is keyed on the destination address, so a hit establishes the
         * first of those. */
        ...
        /* Section 4.3 requires the IA Address option to match the address being registered as well. This
         * is a separate condition from the destination address matched by the lookup above: the option is
         * server-supplied, the destination address is not. */
        while (offset < len) {
