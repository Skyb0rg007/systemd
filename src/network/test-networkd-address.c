...
TEST(dhcp6_address_registration_eligibility) {
        ...
        /* RFC 9686 section 4.2 registers every valid address of global scope and carves out exactly one
         * exclusion: "The client MUST NOT send the ADDR-REG-INFORM message for addresses configured by
         * DHCPv6." Addresses set up by the kernel (FOREIGN) or over D-Bus (RUNTIME) are ordinary global
         * addresses on the link, so they are registered like any other. */
