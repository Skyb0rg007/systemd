...
static int print_configuration(DNSConfiguration *configuration, StatusMode mode, bool *empty_line) {
        ...
        if (mode == STATUS_DNS) {
                ...
                return status_print_strv(configuration, configuration->pref64);
        ...
        if (configuration->ifindex > 0 && !strv_isempty(configuration->pref64)) {
                r = table_add_many(table,
                                   TABLE_FIELD, "NAT64 Prefixes (DNS64)",
                                   TABLE_STRV_WRAPPED, configuration->pref64);
                if (r < 0)
                        return table_log_add_error(r);
        }
...
VERB(verb_pref64, "pref64", "[LINK [PREFIX…]]\0", VERB_ANY, VERB_ANY, 0,
     "Get/set PREF64 (NAT64 prefixes) on an interface for DNS64 synthesis");
static int verb_pref64(int argc, char *argv[], uintptr_t _data, void *userdata) {
        ...
        r = sd_varlink_connect_address(&vl, "/run/systemd/resolve/io.systemd.Resolve");
        if (r < 0)
                return log_error_errno(r, "Failed to connect to resolve service /run/systemd/resolve/io.systemd.Resolve: %m");

        char **prefixes = argc == 3 && isempty(argv[2]) ? STRV_EMPTY : argv + 2;

        r = varlink_callbo_and_log(
                        vl,
                        "io.systemd.Resolve.SetLinkDNS64Prefixes",
                        /* ret_reply= */ NULL,
                        SD_JSON_BUILD_PAIR_INTEGER("ifindex", arg_ifindex),
                        SD_JSON_BUILD_PAIR_BOOLEAN("allowInteractiveAuthentication", arg_ask_password),
                        SD_JSON_BUILD_PAIR_STRV("prefixes", prefixes));
        if (r < 0)
                return r;
